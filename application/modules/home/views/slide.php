<style>
h3{font-size:15px;}
</style>
	<!-- start banner -->
    <div class="banner">
    	 <img src="<?php echo get_image('banner.png');?>" alt="indotes.com" />
    	 <h2>Halo <?php echo ucfirst($c->get_username()); ?>? Ayo Mulai Ujian Sekarang</h2>
    	 <h3>Kerjakan soal...Dapatkan Nilai Terbaikmu!</h3>
    </div>
    <!-- start content -->
	 <div class="main">
	 	 <div class="content">
	 	 	<h2>Pilih Sesi</h2>
	 	 </div>
	 	 <table border=1 style="border:0px solid; width:100%; height:300px;" >
	 	 <tr>
	 	  <td>
	 	   <div class="grid_1_of_4 images_1_of_4" style="width:200px; height:300px;">
			<a href="<?php echo get_site_url();?>">
			<img src="<?php echo get_image('pic2.jpg');?>" title="" alt="indotes.com"/></a>
			 <h3><a href="<?php echo get_site_url();?>">Referensi</a></h3>
			 <p style="text-align:center;"></p>
		   </div>
		  </td>
		  <td>
	 	   <div class="grid_1_of_4 images_1_of_4" style="width:200px; height:300px;" id="opener">
			<a href="<?php echo get_site_url('online_test');?>">
			 <img src="<?php echo get_image('pic1.jpg');?>" title="" alt="indotes.com"/></a>
			 <h3><a href="<?php echo get_site_url('online_test');?>">Start Ujian</a></h3>
			 <p style="text-align:center;"></p>
		   </div>
		  </td>
		  <td>
	 	   <div class="grid_1_of_4 images_1_of_4" style="width:200px; height:300px;">
			<a href="<?php echo get_site_url('browse_scores');?>"><img src="<?php echo get_image('pic3.jpg');?>" title="" alt="indotes.com"/></a>
			 <h3><a href="<?php echo get_site_url('browse_scores');?>">Nilai Ujian</a></h3>
			 <p style="text-align:center;"></p>
		   </div>
		  </td>
		  <td>
	 	   <div class="grid_1_of_4 images_1_of_4" style="width:200px; height:300px;">
			<a href="<?php echo get_site_url();?>"><img src="<?php echo get_image('pic4.jpg');?>" title="" alt="indotes.com"/></a>
			 <h3><a href="<?php echo get_site_url();?>" style="text-indent:20px;">Forum Diskusi</a></h3>
			 <p style="text-align:center;"></p>
		   </div>
		  </td>
		 </tr>
		 </table>

 	 </div>
</div>
</div>

